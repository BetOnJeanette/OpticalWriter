use crate::data::project_bin::{FileData, ProjectBinData};
...
pub fn request_new_file<R: Runtime>(selected_folder_id: String, app_handle: AppHandle<R>) {
    app_handle.dialog().file().pick_files(move |picked_files| {
        let Some(safe_files) = picked_files else {
            return;
        };
        if safe_files.len() == 0 {
            return;
...
pub fn add_files_to_bin<R: Runtime>(
    files: Vec<FilePath>,
    ...
    for file in files {
        ...
        existing_files
            .lock()
            .unwrap()
            .insert(file_id.clone(), file_val.clone());
