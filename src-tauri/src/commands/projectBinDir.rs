use tauri::{State, AppHandle, Manager, Runtime, Emitter};
use serde::Serialize;
use uuid::Uuid;
...
pub fn add_folder<R: Runtime>(app_handle: AppHandle<R>, selected_dir_id: String){
    ...
    cur_dir_state.lock().unwrap().insert(new_id.clone(), new_fold.clone());
    let mut container  = cur_dir_state.lock().unwrap().get(&parent_dir_id).unwrap().clone();
    container.contained_folders.push(new_id.clone());
    cur_dir_state.lock().unwrap().insert(parent_dir_id.clone(), container);
